...
/**
 * Classe représentant une requête OR capable de calculer sa similarité avec un
...
public class OrQuery extends AndOrQuery {
	...
	/**
	 * Construit une requête OR à partir de la requête textuelle fournie en
	...
	public double calculateSimilarity(Index index, Document document) {
		...
		for (Query subQuery : subQueries) {
			sim += Math.pow(subQuery.calculateSimilarity(index, document), P);
		}

		sim = Math.pow(sim / subQueries.size(), 1. / P);
